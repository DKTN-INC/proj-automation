...
from typing import Dict, List, Optional, Any, Callable, TypeVar, Generic, AsyncContextManager
...
class ResourceManager(Generic[T]):
    ...
    async def get_resource(self, key: str, factory: Callable[[], T]) -> T:
        ...
        async with self._lock:
            ...
            # Create new resource
            try:
                resource = factory() if not asyncio.iscoroutinefunction(factory) else await factory()
        ...
        return {
            ...
            "sessions": {
                key: {
                    ...
                    "last_used": meta.get("last_used", "").isoformat() if meta.get("last_used") else "",
    ...
    async def check_memory_pressure(self) -> bool:
        ...
        try:
            ...
            if memory_mb > self.gc_threshold_mb:
                logger.warning(f"Memory pressure detected: {memory_mb:.1f}MB > {self.gc_threshold_mb}MB")

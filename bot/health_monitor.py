    ...
    def get_health_report(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "summary": {
                ...
                "healthy": sum(
                    1 for m in self.metrics.values() if m.status == "healthy"
                ),
                "warnings": sum(
                    1 for m in self.metrics.values() if m.status == "warning"
                ),
                "critical": sum(
                    1 for m in self.metrics.values() if m.status == "critical"
                ),
            },
        }

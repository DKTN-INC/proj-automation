...
def summarize_budget(
    ...
    if not transactions:
        ...
        return BudgetSummary(
            totals=Decimal("0"), remaining=limit_decimal, percent_used=0.0, status="unused"
        )
    ...
    percent_used = float(
        (total_spent / limit_decimal * 100).quantize(Decimal("0.1"), rounding=ROUND_HALF_UP)
    )
    ...
    return BudgetSummary(
        totals=total_spent, remaining=remaining, percent_used=percent_used, status=status
    )


def categorize_spend(
    items: list[dict[str, Any]], category_key: str = "category", amount_key: str = "amount"
) -> dict[str, Decimal]:
    """

    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            with LogContext(
                ...
                logger.info(f"Command '{command_name}' started", extra={
                ...
                try:
                    ...
                    logger.info(f"Command '{command_name}' completed successfully", extra={
                    ...
                    duration = int((time.time() - start_time) * 1000)
                    logger.error(f"Command '{command_name}' failed: {e}", extra={
...
def log_bot_event(logger: logging.Logger, event: str, **extra):
    ...
    logger.info(f"Bot event: {event}", extra={
        'event_type': 'bot_event',
        **extra
    })


def log_api_call(logger: logging.Logger, api: str, endpoint: str, success: bool, duration_ms: int = None, **extra):
    """Log an API call with structured context."""
    logger.info(f"API call to {api} {endpoint} {'succeeded' if success else 'failed'}", extra={
        'event_type': 'api_call',
        'api': api,
        'endpoint': endpoint,
        'success': success,

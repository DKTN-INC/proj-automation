...
class CooldownManager:
    ...
    def __init__(self):
        self._cooldowns: Dict[str, Dict[int, float]] = defaultdict(dict)  # command -> user_id -> next_allowed_time
        ...
        while True:
            try:
                ...
                for command in list(self._cooldowns.keys()):
                    expired_users = [
                        user_id for user_id, next_time in self._cooldowns[command].items()
    ...
    def add_cooldown(self, user_id: int, command: str, cooldown_seconds: float):
        ...
        logger.debug(f"Added cooldown for user {user_id} on command '{command}' for {cooldown_seconds}s")

    ...
    if os.path.exists(_TASKS_FILE):
        try:
            with open(_TASKS_FILE, "r") as f:
                _tasks = json.load(f)
                if _tasks:
                    _next_task_id = max(task["id"] for task in _tasks) + 1
                else:
                    _next_task_id = 1
        except (json.JSONDecodeError, TypeError):
            logger.warning(f"Could not decode '{_TASKS_FILE}'. Backing up and starting fresh.")
            os.rename(_TASKS_FILE, f"{_TASKS_FILE}.bak")
            _tasks = []
            _next_task_id = 1
    else:
        _tasks = []
